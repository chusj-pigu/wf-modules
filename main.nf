...
process QUARTO_PLOT_NANOCOV_COHORT {
    container 'ghcr.io/chusj-pigu/quarto:1aae9c5de71b00a19debb6db4067e5eb67301126'
    tag "$meta.id"
    label 'process_low'
    label 'process_single_cpu'
    ...
    script:
    def prefix    = task.ext.prefix ?: "${meta.id}"
    def input_dir = "${prefix}_${section}_${process_name}_inputs"
    def file_list = (statistics_files instanceof List ? statistics_files : [statistics_files])
    def cp_cmds   = file_list.collect { "cp -L ${it} ${input_dir}/" }.join('\n')

    """
    mkdir -p ${input_dir}
    ${cp_cmds}

    cat <<'END_REPORT' > ${input_dir}/${prefix}-${section}-${process_name}.qmd
\`\`\`{r}
#| label: ${prefix}-${section}-${process_name}-global
#| fig-cap: "Mean coverage per sample across the cohort"
#| echo: false
library(vroom)
library(ggplot2)
library(dplyr)
...
if (length(files) == 0) {
  message("No .statistics files found")
} else {
  dat <- map_dfr(files, function(f) {
    d <- vroom(f, show_col_types = FALSE)
    d\$sample <- sub("\\\\.statistics\$", "", basename(f))
    d
  })
  p <- ggplot(dat, aes(x = reorder(sample, mean_coverage), y = mean_coverage)) +
    geom_col(fill = "#2196F3", alpha = 0.85) +
    coord_flip() +
    labs(x = NULL, y = "Mean Coverage (×)",
         title = "Global Coverage Summary") +
...
process QUARTO_PLOT_NANOCOV_REGIONS_COHORT {
    container 'ghcr.io/chusj-pigu/quarto:1aae9c5de71b00a19debb6db4067e5eb67301126'
    tag "$meta.id"
    label 'process_low'
    label 'process_single_cpu'
    ...
    def prefix    = task.ext.prefix ?: "${meta.id}"
    def input_dir = "${prefix}_${section}_${process_name}_inputs"
    def file_list = (regions_files instanceof List ? regions_files : [regions_files])
    def cp_cmds   = file_list.collect { "cp -L ${it} ${input_dir}/" }.join('\n')
    ...
    cat <<'END_REPORT' > ${input_dir}/${prefix}-${section}-${process_name}.qmd
\`\`\`{r}
#| label: ${prefix}-${section}-${process_name}-regions
#| fig-cap: "Mean target-region coverage per sample"
#| echo: false
library(vroom)
library(ggplot2)
library(dplyr)
library(purrr)
files <- list.files(pattern = "\\\\.regions\\\\.statistics\\\\.tsv\$")
if (length(files) == 0) {
  message("No .regions.statistics.tsv files found")
} else {
  dat <- map_dfr(files, function(f) {
    d <- vroom(f, show_col_types = FALSE)
    d\$sample <- sub("\\\\.regions\\\\.statistics\\\\.tsv\$", "", basename(f))
    d
  })
  summary_dat <- dat |>
    group_by(sample) |>
    summarise(
      median_region_coverage = median(mean_coverage, na.rm = TRUE),
      mean_region_coverage   = mean(mean_coverage, na.rm = TRUE),
      .groups = "drop"
    )
  p <- ggplot(summary_dat, aes(x = reorder(sample, median_region_coverage),
                                y = median_region_coverage)) +
    geom_col(fill = "#4CAF50", alpha = 0.85) +
    coord_flip() +
    labs(x = NULL, y = "Median Region Coverage (×)",
         title = "Target Region Coverage Summary") +
    theme_minimal(base_size = 12)
  print(p)
}
\`\`\`
