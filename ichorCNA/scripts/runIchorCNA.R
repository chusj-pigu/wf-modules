...
option_list <- list(
  ...
  make_option(c("--seqInfoData"), type = "character", default=NULL, help = "SeqInfo saved file to use. Default: [%default]"),
  make_option(c("--downloadOnly"), type="logical", default=FALSE, help="Run ichorCNA offline. Default: [%default]")
)
...
if (is.null(opt$seqInfoData) ){
  seqinfo <- getSeqInfo(genomeBuild, genomeStyle)
  save(seqinfo, file = paste0(outDir,"/seqinfo.RData"))
  message("Saved seqinfo to ", paste0(outDir,"/seqinfo.RData"))
  if( opt$downloadOnly ){
    message("Ran in download only mode - Exiting ichorCNA")
    q()
  }
} else {
  load(opt$seqInfoData)
  message("Loaded seqinfo from ", opt$seqInfoData)
}
